...
public class RandomMapGen : MonoBehaviour
{
    ...
    Mesh mesh;
    [SerializeField] private int MESH_SCALE = 1;
    ...
    {
        float amplitude = 20; // Alkuperäisesti 20
        float frequency = 1; // Alkuperäisesti 1
        float persistence = 0.5f; // Alkuperäisesti 0.5f
        float noiseHeight = 0; // Alkuperäisesti 0
                ...
                if (noiseHeight > minSpawnHeight)
                {
                    ...
                    if (Random.Range(1, 5) == 1)
                    {
                        if (objects.Any())
                        {
                            ...
                            GameObject newGO = Instantiate(objectToSpawn, new Vector3(mesh.vertices[i].x * MESH_SCALE, spawnAboveTerrainBy, mesh.vertices[i].z * MESH_SCALE), Quaternion.identity);
